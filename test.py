...
if not _HAS_TLS_CLIENT:
    ...
    if not _HAS_CURL_CFFI:
        raise SystemExit(
            "❌ tls_client veya curl_cffi gerekli: pip install tls-client yada pip install curl_cffi"
        )
else:
    _HAS_CURL_CFFI = False
...
if _HAS_TLS_CLIENT:
    class _HTTPClient:
        def __init__(self, proxy, timeout, client_token):
            self.proxy = proxy
        ...
        def request(self, method, url):
            ...
            return self.session.get(url, timeout_seconds=self.timeout)
...
class KickAPIClient:
    def __init__(self, client_token: str, proxy: Optional[ProxyEntry] = None, timeout: int = 25):
        self.client_token = client_token
        self.proxy = proxy
        self.timeout = timeout
        self._client: Optional[_HTTPClient] = None
        self._create_session()
    
    def _create_session(self):
        self._client = _HTTPClient(self.proxy, self.timeout, self.client_token)
    
    async def fetch_token(self) -> str:
        def _fetch():
            resp = self._client.request("GET", "https://websockets.kick.com/viewer/v1/token")
    ...
    async def fetch_channel_info(self, channel: str) -> StreamInfo:
        def _fetch():
            resp = self._client.request_json(f"https://kick.com/api/v2/channels/{channel}")
            if resp.status_code >= 400:
                raise RuntimeError(f"Channel request failed: {resp.status_code}")
            data = resp.json()
