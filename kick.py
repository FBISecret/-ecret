...
def get_channel_info(name):
    ...
    try:
        ...
        if channel_id:
            print(f"Kanal kimliği: {channel_id}")
        if stream_id:
            print(f"Akış kimliği: {stream_id}")
...
def show_stats():
    ...
    while not stop:
        try:
            ...
            print("\033[3A", end="")
            print(f"\033[2K\r[x] Bağlantılar: \033[32m{ws_count}\033[0m | denemeler: \033[32m{ws_attempts}\033[0m")
            print(f"\033[2K\r[x] Ping'ler: \033[32m{ping_count}\033[0m | Kalp atışları: \033[32m{heartbeat_count}\033[0m | Süre: \033[32m{duration}\033[0m | Akış kimliği: \033[32m{stream_display}\033[0m")
            print(f"\033[2K\r[x] İzleyiciler: \033[32m{viewer_display}\033[0m | Güncellendi: \033[32m{time.strftime('%H:%M:%S', time.localtime(last_check))}\033[0m")
            sys.stdout.flush()
            time.sleep(1)
        except:
            time.sleep(1)
...
if __name__ == "__main__":
    try:
        os.system('cls' if os.name == 'nt' else 'clear')
        channel_input = input("Kanal adını veya URL'sini girin: ").strip()
        if not channel_input:
            print("Kanal adı gerekli.")
            sys.exit(1)
        
        while True:
            try:
                thread_input = int(input("İzleyici sayısını girin: ").strip())
                if thread_input > 0:
                    break
                else:
                    print("0'dan büyük olmalıdır.")
            except ValueError:
